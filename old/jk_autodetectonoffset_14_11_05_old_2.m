function segments = jk_autodetectonoffset_14_11_05(segments,ind)
...
%     hold off;
    ...
    threshold = 2*mean(abs(diff(dataOut(2:round(2+(size(diff(dataOut),2)/10)))))+ ...
        abs(diff(dataOut(round(end-(size(diff(dataOut),2)/10)):end)))) + ...
        std(abs(diff(dataOut(2:round(2+(size(diff(dataOut),2)/10)))))+ ...
        abs(diff(dataOut(round(end-(size(diff(dataOut),2)/10)):end))));
%     threshold = 2*mean(abs(diff(dataOut(2:round(2+(size(diff(dataOut),2)/10)))))+ ...
%         abs(diff(dataOut(round(end-(size(diff(dataOut),2)/10)):end))));
