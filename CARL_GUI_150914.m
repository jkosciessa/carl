classdef CARL_GUI_150914 < handle
    ...
    methods (Access = 'private')
        ...
        function [hObject, this] = createFigure(this)
            ...
            this.ChannelHandles = plot(this.AxesHandles, currentdata(1:end));
            ...
            set(this.FigureHandle,'Name','Audio Analysis GUI')
    ...
    methods (Access = 'public', Hidden = true) % Callback methods
        ...
        function shiftLeft_Callback(this, ~, ~)                       % shift segment by X timepoints to the left
            ...
            if ind ~= 1
                if max(this.AudioData{2,ind-1}) >= tmp.newonset
                    ...
                    [this.AudioData{1,ind-1},~] = audioread([pdat.dataCon, this.ID,'.wav'], ...
                        [tmp.prevonset tmp.offset]); % reload data
                    ...
                    % update segments structure
                    [this.AudioData{1,ind},~] = audioread([pdat.dataCon, this.ID,'.wav'], ...
                        [tmp.newonset tmp.offset]); % reload data
                    this.AudioData{2,ind} = [tmp.newonset:tmp.offset]';
                    if ~isempty(this.AudioData{5,ind}) && ~isempty(this.AudioData{7,ind})
                        tmp.findNewLocMarked = find(this.AudioData{2,ind} == min(this.AudioData{7,ind}));
                        % save all values:
                        % tmp.NewLocsMarked = [tmp.findNewLocMarked:tmp.findNewLocMarked+size(this.AudioData{5,ind},1)]';
                        % save only first and last value:
                        tmp.NewLocsMarked = [tmp.findNewLocMarked;tmp.findNewLocMarked+size(this.AudioData{5,ind},1)]';
                        this.AudioData{5,ind} = tmp.NewLocsMarked;
                    end;
                    % update plot
                    updatePlot(this);
                end;
        ...
        function shiftRight_Callback(this, ~, ~)                            % shift segment by X timepoints to the right
            ...
            if ind ~= size(this.AudioData,2)
                if tmp.newoffset >= min(this.AudioData{2,ind+1})
                    ...
                    [this.AudioData{1,ind+1},~] = audioread([pdat.dataCon, this.ID,'.wav'], ...
                        [tmp.onset tmp.nextoffset]); % reload data
                    this.AudioData{2,ind+1} = [tmp.onset:tmp.nextoffset]';
                    this.AudioData{4,ind+1} = [];       % erase previous coding if available
                    if ~isempty(this.AudioData{5,ind+1}) && ~isempty(this.AudioData{7,ind+1})
                        tmp.findNewLocMarked = find(this.AudioData{2,ind+1} == min(this.AudioData{7,ind+1}));
                        % save all values:
                        % tmp.NewLocsMarked = [tmp.findNewLocMarked:tmp.findNewLocMarked+size(this.AudioData{5,ind},1)]';
                        % save only first and last value:
                        tmp.NewLocsMarked = [tmp.findNewLocMarked;tmp.findNewLocMarked+size(this.AudioData{5,ind},1)]';
                        this.AudioData{5,ind+1} = tmp.NewLocsMarked;
                    end;
                    this.AudioData(:,ind)   = [];       % delete now obsolete trial
                    ...
                    % update segments structure
                    [this.AudioData{1,ind},~] = audioread([pdat.dataCon, this.ID,'.wav'], ...
                        [tmp.onset tmp.newoffset]); % reload data
        ...
        function playbutton_Callback(this, ~, ~)
            ...
            if strcmp(this.version, 'Checking')
                ...
                player = audioplayer(this.AudioData{1,ind}(tmp.begin:tmp.end), this.Fs);
                play(player)
                lineobj = line([tmp.begin,tmp.begin],[-1,1],'color','r', 'linewidth', 1);
                end_time = length(this.AudioData{1,ind}(tmp.begin:tmp.end))/this.Fs;
            else
                player = audioplayer(this.AudioData{1,ind}(1:end), this.Fs);
                play(player)
...
%             updatePlot(this);
    ...
    %           this.Analyzer.analyze(data, this.Fs);
        ...
        function [xd1, xd2] = getSelectionSampleNumbers(this)
            ...
            this.AudioData{5,ind} = round(xd1:xd2)';
    ...
    methods (Access = 'public', Hidden = false)
        ...
        function updatePlot(this)
            ...
            this.ChannelHandles = plot(this.AxesHandles, currentAudio);
            xlim([0 max(this.AudioData{2,ind}(1:end)-this.AudioData{2,ind}(1))]);
