function segments = CARL_AutoDetectOnOffset_150911(speechSignal,ind)
...
% short-time energy is used, downsampled and low-pass-filtered.
    ...
    winLen = round(size(speechSignal,1)/800)+1;                          % specify hamming window
    winOverlap = round(size(speechSignal,1)/800);
...
%     hold off;
    ...
    a = 1;
    sampling_factor = round(size(speechSignal,1)/100);
    ...
    if max_absdiff + 2*sampling_factor < size(speechSignal,1)
